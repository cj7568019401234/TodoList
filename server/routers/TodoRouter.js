...
router.get('/todo', async (ctx, next) => {  //添加url-route，get请求
    ctx.response.body = await todoManager.getTodo();   //异步查询TodoList
})

router.post('/todo/update', async (ctx, next) => {  //添加url-route，post请求
    let item = ctx.request.body.todo;   //获取请求中需要修改的todo对象
    console.log(item);
    ctx.response.body = await todoManager.updateTodo(item);   //异步修改TodoList
});
...
router.post('/todo/toggle', async (ctx, next) => {
    ...
    console.log(id);
    ctx.response.body = await todoManager.toggleTodo(id);   //异步删除TodoList
});
